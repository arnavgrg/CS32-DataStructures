...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
	...
	{
		int j = i;
		//shift the items as needed
		while ((j > 0) && comp(s[i], s[j - 1]))
		{
			s[j] = s[j - 1];
			j--;
		}
		//store the swaped value into the vacated slot 
		s[j] = s[i];
	}
}
...
int main()
{
	...
	// destruction time.
	{
		...
		//Created vector of sensor pointers and get each pointer to point to a sensor object 
		...
		//set sensors to the sorted sensors 
